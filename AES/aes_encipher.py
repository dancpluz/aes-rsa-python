from utils import matrix_to_state, state_to_matrix

S_BOX = bytes([
  
])

INV_S_BOX = bytes([
  
])

# Substitui cada byte do estado pelo byte correspondente na S_BOX
def sub_bytes(state):
    return bytes(S_BOX[byte] for byte in state)

# Desloca as linhas do estado
def shift_rows(state):
    matrix = state_to_matrix(state)
    for i in range(4):
        matrix[i] = matrix[i][i:] + matrix[i][:i]
    return matrix_to_state(matrix)

# Multiplica dois polinômios no campo finito GF(2^8)
def mul_gf(a, b):
    p = 0
    for _ in range(8):
        if b & 1:
            p ^= a
        carry = a & 0x80
        a <<= 1
        if carry:
            a ^= 0x1b
        b >>= 1
    return p & 0xFF

# Aplica MixColumns em uma única coluna
def mix_single_column(column):
    c_0 = column[0]
    c_1 = column[1]
    c_2 = column[2]
    c_3 = column[3]

    column[0] = mul_gf(c_0, 0x02) ^ mul_gf(c_1, 0x03) ^ c_2 ^ c_3
    column[1] = c_0 ^ mul_gf(c_1, 0x02) ^ mul_gf(c_2, 0x03) ^ c_3
    column[2] = c_0 ^ c_1 ^ mul_gf(c_2, 0x02) ^ mul_gf(c_3, 0x03)
    column[3] = mul_gf(c_0, 0x03) ^ c_1 ^ c_2 ^ mul_gf(c_3, 0x02)

    return column

def mix_columns(state):
    matrix = state_to_matrix(state)
    for i in range(4):
        column = [row[i] for row in matrix]
        column = mix_single_column(column)
        for j in range(4):
            matrix[j][i] = column[j]
    return matrix_to_state(matrix)

# Rota os bytes de uma palavra (4 bytes) para a esquerda.
def rot_word(word):
    return word[1:] + word[:1]

# Substitui cada byte de uma palavra pela S-Box.
def sub_word(word):
    return bytes(S_BOX[b] for b in word)

RCON = bytes([
    0x01, 0x02, 0x04, 0x08, 0x10, 
    0x20, 0x40, 0x80, 0x1B, 0x36,
    0x6C, 0xD8, 0xAB, 0x4D, 0x9A
])

# Retorna o valor da constante de rodada Rcon[i].
def rcon(i):
    return bytes([RCON[i], 0x00, 0x00, 0x00])

# Gera as chaves de rodada a partir da chave original.
def key_expansion(key, num_rounds):
    nk = 4  # Número de palavras na chave original (para AES-128, são 4 palavras de 32 bits)
    nb = 4  # Número de palavras por bloco (também 4 para AES-128)
    expanded_key = []

    for i in range(nk):
        expanded_key.append(key[4*i:4*(i+1)])

    for i in range(nk, nb * (num_rounds + 1)):
        temp = expanded_key[i-1]

        if i % nk == 0:
            temp = sub_word(rot_word(temp))
            temp = bytes(t ^ r for t, r in zip(temp, rcon(i // nk - 1)))
        
        expanded_key.append(bytes(t ^ p for t, p in zip(expanded_key[i - nk], temp)))

    return b''.join(expanded_key)

# Adiciona a chave da rodada ao estado usando XOR.
def add_round_key(state, key):
    state_matrix = state_to_matrix(state)
    key_matrix = state_to_matrix(key)
    
    # Aplica a operação XOR entre o estado e a chave da rodada
    for r in range(4):
        for c in range(4):
            state_matrix[r][c] ^= key_matrix[r][c]
    
    return matrix_to_state(state_matrix)

# Cifra o texto usando AES-128 com o número especificado de rodadas.
def aes_encipher(plaintext, key, num_rounds=10):
    state = plaintext
    round_keys = key_expansion(key, num_rounds)

    state = add_round_key(state, round_keys[:16])
    # Rodadas principais
    for round in range(1, num_rounds):
        state = sub_bytes(state)
        state = shift_rows(state)
        state = mix_columns(state)
        state = add_round_key(state, round_keys[round * 16: (round + 1) * 16])

    # Rodada final
    state = sub_bytes(state)
    state = shift_rows(state)
    state = add_round_key(state, round_keys[num_rounds * 16: (num_rounds + 1) * 16])

    return state